...
<head>
    ...
    <title>注册-个人用户</title>
    <link type="text/css" rel="stylesheet" href="/taotao-sso/css/regist.personal.css"/>
    <link type="text/css" rel="stylesheet" href="/taotao-sso/css/passport.base.css"/>
    <script type="text/javascript" src="/taotao-sso/js/jquery-1.6.4.js"></script>
</head>
<body>
<div class="w" id="logo">
    <div>
    	<a href="http://localhost:8082/taotao-portal">
    		<img src="/taotao-sso/images/taotao-logo.gif" alt="淘淘商城" width="170" height="60"/>
    	</a> <b></b>
    </div>
</div>
...
<div class="w" id="regist">
    <div class="mt">
        ...
        <div class="extra">
        <span>我已经注册，现在就&nbsp;
        	<a href="/taotao-sso/login" class="flk13">登录</a>
        </span>
        </div>
    </div>
    <div class="mc">
        <form id="personRegForm" method="post" onsubmit="return false;">
            ...
            <div class="phone">
                <img width="180" height="180" src="/taotao-sso/images/phone-bg.jpg">
            </div>
            <span class="clr"></span>
        </form>
    </div>
<script type="text/javascript">
	...
	var REGISTER={
		...
		doSubmit:function() {
			$.post("/taotao-sso/user/register",$("#personRegForm").serialize(), function(data){
				if(data.status == 200){
					
					REGISTER.login();
				} else {
					if(data.dataBean){
						if(data.dataBean.username){
							$("#regName_error").removeClass("hide").val(data.dataBean.username);
						}
						if(data.dataBean.phone){
						   alert("ddd");
							$("#phone_error").html(data.dataBean.phone).css("color","red");
						}
					}
					alert("注册失败！");
				}
		...
		login:function() {
		  if(confirm("用户注册成功，是否返回登录页面")){
			 location.href = "/taotao-sso/login";
		  }
			 return false;
		},
		reg:function() {
